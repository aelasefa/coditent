...
| Profile photo | Select an image up to 2 MB; photo is included with profile save | Refresh current user data | Keep draft and show error feedback |
...
Shared ownership: `apps/web/src/components/ui/` provides form fields, buttons, dialog, sheet, skeleton, and toast; `apps/web/src/components/shell/` provides desktop/mobile navigation; `apps/web/src/app/globals.css` owns semantic tokens, focus, and motion defaults. Candidate-specific layout lives in `apps/web/src/components/candidate/candidate-pages.module.css`. The profile tabs implement the tablist arrow-key/Home/End convention. Mobile navigation remains available at 375px; the application stage row is horizontally scrollable rather than compressed. Job details stay beside results on desktop and open in a sheet below 1024px.
